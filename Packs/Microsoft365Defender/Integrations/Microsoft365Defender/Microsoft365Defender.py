...
def _get_meta_data_for_incident(raw_incident: dict) -> dict:
    ...
    for alert in alerts_list:
        for entity in alert.get("entities", []):
            if entity.get("entityType") == "Mailbox" and entity.get("mailboxAddress", "") not in mailboxes_set:
                mailboxes.append(
                    {"Mailbox": entity.get("mailboxAddress", ""), "Display Name": entity.get("mailboxDisplayName", "")}
                )
    ...
    return {
        ...
        "First activity": str(min(first_activity_list, key=lambda x: dateparser.parse(x))) if alerts_list else "",  # type: ignore
        "Last activity": str(max(last_activity_list, key=lambda x: dateparser.parse(x))) if alerts_list else "",  # type: ignore
        "Devices": [
            {
                "device name": device.get("deviceDnsName", ""),
...
def convert_incident_to_readable(raw_incident: dict) -> dict:
    ...
    device_groups = {device.get("device name") for device in incident_meta_data.get("Devices", []) if device.get("device name")}
    return {
...
def microsoft_365_defender_incidents_list_command(client: Client, args: dict) -> CommandResults:
    ...
    response = client.incidents_list(
        limit=limit, status=status, assigned_to=assigned_to, skip=offset, timeout=timeout, odata=odata
    )
    ...
    return CommandResults(
        outputs_prefix="Microsoft365Defender.Incident",
...
def microsoft_365_defender_incident_update_command(client: Client, args: dict) -> CommandResults:
    ...
    human_readable_table = tableToMarkdown(
        name=f"Updated incident No. {incident_id}:", t=readable_incident, headers=list(readable_incident.keys())
    )
...
def microsoft_365_defender_incident_get_command(client: Client, args: dict) -> CommandResults:
    ...
    human_readable_table = tableToMarkdown(
        name=f"Incident No. {incident_id}:", t=readable_incident, headers=list(readable_incident.keys())
    )
...
def microsoft_365_defender_advanced_hunting_command(client: Client, args: dict) -> CommandResults:
    ...
    human_readable_table = tableToMarkdown(name=f" Result of query: {query}:", t=results, headers=headers)
    return CommandResults(
...
def fetch_modified_incident_ids(client: Client, last_update_time: str) -> List[str]:
    ...
    while True:
        ...
        response = client.incidents_list(timeout=TIMEOUT_INT, last_update_time=last_update_time, skip=offset)
        raw_incidents = response.get("value")
        demisto.debug(f"Microsoft Defender 365 - fetch_modified_incident_ids - fetched {len(raw_incidents)}")
        for incident in raw_incidents:
...
def get_entries_for_comments(comments: List[dict[str, str]], last_update: datetime, comment_tag: str) -> List[dict]:
    """
...
def get_incident_entries(
    mirrored_object: dict, last_update: datetime, comment_tag: str, close_incident: bool = False
) -> List[dict]:
    """
...
def get_remote_data_command(client: Client, args: dict[str, Any]) -> GetRemoteDataResponse:
    ...
    try:
        ...
        demisto.debug(
            f"Error in Microsoft incoming mirror for incident: {remote_incident_id}\n"
    ...
    for entry in entries:
        ...
        if comment_tag in tags:
            if str(entry.get("format")) == "html":
                ...
                text = (
                    f"({user}): <br/><br/>[code]{contents} <br/><br/>[/code]"
            ...
            client.update_incident(incident_id=remote_incident_id, timeout=TIMEOUT_INT, comment=text)
...
def update_remote_system_command(client: Client, args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if update_remote_system_args.entries:
            ...
            demisto.debug("Microsoft Defender 365 - new entries were found in the xsoar incident.")
